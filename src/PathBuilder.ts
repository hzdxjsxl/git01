import { Vector3, Color3, MeshBuilder, LinesMesh, Mesh, Scene } from 'babylonjs';
import { GcodeModel, GcodeLine, GcodePosition } from './GcodeInterpreter';
...
export class PathBuilder {
  private scene: Scene;
  private options: PathBuilderOptions;
  private renderedPaths: RenderedPath[] = [];

  private static defaultOptions: PathBuilderOptions = {
  ...
  public updateOptions(options: Partial<PathBuilderOptions>): void {
    this.options = { ...this.options, ...options };
  }

  public render(model: GcodeModel, layerRange?: { start: number; end: number }): RenderedPath[] {
    this.dispose();
    ...
    const travelPaths: Vector3[][] = [];
    const rapidPaths: Vector3[][] = [];
    const extrudePaths: Vector3[][] = [];

    for (const layer of layersToRender) {
      const layerTravelPaths: Vector3[][] = [];
      const layerRapidPaths: Vector3[][] = [];
      const layerExtrudePaths: Vector3[][] = [];

      let currentTravelPath: Vector3[] | null = null;
      let currentRapidPath: Vector3[] | null = null;
      let currentExtrudePath: Vector3[] | null = null;
  ...
  private createLineSystem(paths: Vector3[][], color: Color3, lineWidth: number): LinesMesh {
    const options: any = {
      lines: paths,
  ...
  private createRibbon(path: Vector3[]): Mesh | null {
    if (path.length < 2) {
      return null;
    }
  ...
  private positionToVector(pos: GcodePosition): Vector3 {
    return new Vector3(
      pos.x ?? 0,
      pos.z ?? 0,
      pos.y ?? 0
  ...
  public setVisibility(type: 'travel' | 'rapid' | 'extrude', visible: boolean): void {
    for (const path of this.renderedPaths) {
      if (path.type === type) {
  ...
  public getRenderedPaths(): RenderedPath[] {
    return this.renderedPaths;
  }
}
