...
for (let i = 0; i < source.length; i++) {
  ...
  nnErr += best;
  errSum += Math.hypot(a[0]-target[i][0], a[1]-target[i][1], a[2]-target[i][2]);
}
console.log(`Ordered mean error (cheating, order mismatched): ${(errSum/source.length).toFixed(5)}`);
console.log(`NN-mean error (actual alignment quality): ${(nnErr/source.length).toFixed(5)}`);
